...
const Email = ({ value, setValue }) => {
  ...
  async function fetchData() {
    ...
    try {
      const response = await axios.get(
        `${api_url}/user/get-user-data-by-email`,
        {
          headers: {
            email: emailId,
          },
  ...
  async function sendOtp() {
    ...
    (function () {
      emailjs.init("NL09kZCZYeYFmZSgt");
    })();

    let templateParams = {
      to_name: emailId,
      name: user.name,
      from_name: "manimekalai.eee123@gmail.com",
      message_html: msg,
      app: "Notes Taking App",
    };
...
const NewPassword = () => {
  ...
  async function handleResetPassword() {
    if (values.password === values.confirmPassword) {
      ...
      try {
        await axios.put(`${api_url}user/reset-password`, obj);
        navigate("/login");
      } catch (error) {
        console.error("Error In Fetching Data:", error);
      }
